 ...
 *
 * This will set the location the library uses for it's math.
 * The timezone is included in this as it's not valid to call any of the calc functions until you have set a timezone.
 * It is possible to simply call setPosition one time, with a timezone and not use the setTZOffset() function ever, if you never change timezone values.
 * 时区包含在此中，因为在设置时区之前调用任何calc函数都是无效的。
 * 如果您从不更改时区值，则可以简单地调用setPosition一次，而不再使用setTZOffset（）函数。
 *
 ...
 * 这是旧版本的setPosition使用整数时区，不会被弃用。但是，今后最好使用双版本。
 */
